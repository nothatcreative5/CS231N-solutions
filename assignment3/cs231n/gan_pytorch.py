...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss += bce_loss(logits_fake, torch.zeros_like(logits_fake))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    batch_size = len(scores_real)
    loss = 1/(2 * batch_size) * (torch.sum((scores_real - 1)**2)
     + torch.sum((scores_fake)**2))
...
def ls_generator_loss(scores_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    batch_size = len(scores_fake)
    loss = 1/(2 * batch_size) * torch.sum((scores_fake - 1)**2)
